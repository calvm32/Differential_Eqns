...
"""
Program to compare 2 different kinds of errors vs. truncation length,
...
ax1.set_xlabel("truncation length"); ax1.set_ylabel("error")
ax1.set_title("Bisection Method Error Comparison")
ax1.grid(True, which="major", ls="--", alpha=0.5)

ax2.set_xlabel("truncation length"); ax2.set_ylabel("error")
...
for N in range(N_iter):
    ...
    for i in range(1,17):
        trunc_len = i
        B_iter_list.append(trunc_len)
        N_iter_list.append(trunc_len)

        B_zero = truncated_bisection(f, a, b, dps=trunc_len)
        N_zero = truncated_newton(f, f_prime, x0, dps=trunc_len)
...
B_x = np.log10(B_x_list)
log_B_norm_y = np.log10(B_norm_y_list)
log_B_diff_y = np.log10(B_diff_y_list)
N_x = np.log10(N_x_list)
log_N_norm_y = np.log10(N_norm_y_list)
log_N_diff_y = np.log10(N_diff_y_list)

B_norm_slope, B_norm_intercept = np.polyfit(B_x_list, log_B_norm_y, 1)
B_diff_slope, B_diff_intercept = np.polyfit(B_x_list, log_B_diff_y, 1)
N_norm_slope, N_norm_intercept = np.polyfit(N_x_list, log_N_norm_y, 1)
N_diff_slope, N_diff_intercept = np.polyfit(N_x_list, log_N_diff_y, 1)

# Smooth x-values for line
B_x_fit = np.linspace(1, 16, 200)
N_x_fit = np.linspace(1, 16, 200)

B_norm_y_fit = 10**(B_norm_slope * B_x_fit + B_norm_intercept)
B_diff_y_fit = 10**(B_diff_slope * B_x_fit + B_diff_intercept)
N_norm_y_fit = 10**(N_norm_slope * N_x_fit + N_norm_intercept)
N_diff_y_fit = 10**(N_diff_slope * N_x_fit + N_diff_intercept)
